...
def detect_objects_in_images(image_folder, model_path):
    ...
    fracture_color = (0, 0, 255)  # Blue for fractures
    ...
    for image in images:
        ...
        for i in range(object_count):
            ...
            if class_name == 'crack':
                has_crack = True
                mask_color = crack_color
            elif class_name == 'fracture':
                has_fracture = True
                mask_color = fracture_color
            else:
            ...
            for cnt in contours:
                if class_name == 'crack':
                    image = draw_mask(image, [cnt], crack_color)
                elif class_name == 'fracture':
                    image = draw_mask(image, [cnt], fracture_color)

        # 5. Add text to image
        text = ""
        color = (0, 0, 0)
        if has_crack and has_fracture:
            text = "Cracked and Fractured Tile"
        elif has_crack:
            text = "Cracked Tile"
            color = crack_color
        elif has_fracture:
...
# Example usage
image_folder_path = "content/OK_tiles"
#image_folder_path = "content/test"
model_file_path = "content/mask_rcnn_object_0026.h5"
